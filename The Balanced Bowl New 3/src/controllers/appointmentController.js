...
// -----------------------------------------------------
// DIETICIAN: VIEW MY CLIENTS
// -----------------------------------------------------
exports.myClients = async (req, res) => {
  try {
    if (!req.session.user || req.session.user.role !== 'dietician') {
      req.session.error = "Access denied";
      return res.redirect('/');
    }

    // Find this dietician's ID
    const dietician = await Dietician.findOne({ user: req.session.user.id }).lean();
    if (!dietician) {
      req.session.error = "Dietician profile not found";
      return res.redirect('/profile');
    }

    // Get all appointments for the dietician
    const clients = await Appointment.find({ dietician: dietician._id })
      .populate('user', 'name email phone')
      .lean();

    res.render('my-clients', { clients });

  } catch (err) {
    console.error("MY CLIENTS ERROR:", err);
    req.session.error = "Unable to load clients";
    res.redirect('/');
  }
};
